...
HandType extractHandType(const Hand &h) {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        if (count == 5) return HandType::FIVE;
            ...
            if (jCount == 1 || jCount == 4) return HandType::FIVE;
            else return HandType::FOUR;
        }
        else if (count == 3 && h.cards[i] != 'J') haveTriple = true;
        else if (count == 2) {
            if (firstDouble == 0 && h.cards[i] != 'J') {
                haveDouble = true;
                firstDouble = h.cards[i];
            }
            else if (h.cards[i] != firstDouble && h.cards[i] != 'J') {
                if (jCount == 1) return HandType::FH;
                else return HandType::TWO;
            }
        }
    }
    ...
    if (haveTriple) {
        ...
        else if (jCount == 2) return HandType::FIVE;
        else if (jCount == 1) return HandType::FOUR;
        else return HandType::THREE;
    }
    else if (haveDouble) {
        if (jCount == 3) return HandType::FIVE;
        else if (jCount == 2) return HandType::FOUR;
        else if (jCount == 1) return HandType::THREE;
        else return HandType::ONE;
    }
    else if (jCount == 5 || jCount == 4) return HandType::FIVE;
    else if (jCount == 3) return HandType::FOUR;
    else if (jCount == 2) return HandType::THREE;
    else if (jCount == 1) return HandType::ONE;
